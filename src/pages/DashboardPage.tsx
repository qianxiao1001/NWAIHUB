...
const Card = ({ title, children, className, icon: Icon }: { title: string, children: React.ReactNode, className?: string, icon?: any }) => (
  <div className={cn("relative bg-slate-900/60 border border-slate-800 backdrop-blur-sm rounded-xl overflow-hidden flex flex-col", className)}>
    {/* Tech borders */}
    <div className="absolute top-0 left-0 w-2 h-2 border-t-2 border-l-2 border-blue-500/50 rounded-tl-sm" />
    <div className="absolute top-0 right-0 w-2 h-2 border-t-2 border-r-2 border-blue-500/50 rounded-tr-sm" />
    <div className="absolute bottom-0 left-0 w-2 h-2 border-b-2 border-l-2 border-blue-500/50 rounded-bl-sm" />
    <div className="absolute bottom-0 right-0 w-2 h-2 border-b-2 border-r-2 border-blue-500/50 rounded-br-sm" />
    
    <div className="px-4 py-3 border-b border-slate-800/80 flex items-center gap-2 bg-slate-900/40">
      {Icon && <Icon className="w-4 h-4 text-blue-400" />}
      <h3 className="text-slate-100 font-semibold tracking-wide text-sm">{title}</h3>
      <div className="ml-auto w-12 h-[1px] bg-gradient-to-r from-transparent via-blue-500/50 to-transparent" />
    </div>
    <div className="p-4 flex-1 overflow-hidden relative">
      {children}
      {/* Scanline effect */}
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-blue-500/[0.02] to-transparent pointer-events-none animate-scan" />
    </div>
  </div>
);
...
const MapVisualization = () => {
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {/* Decorative rings */}
      <div className="absolute w-[500px] h-[500px] border border-blue-500/10 rounded-full animate-[spin_60s_linear_infinite]" />
      <div className="absolute w-[400px] h-[400px] border border-dashed border-cyan-500/20 rounded-full animate-[spin_40s_linear_infinite_reverse]" />
      <div className="absolute w-[300px] h-[300px] border border-blue-400/10 rounded-full" />
      
      {/* Abstract Map SVG (Conceptual Nanjing) */}
      <svg viewBox="0 0 400 400" className="w-[80%] h-[80%] drop-shadow-[0_0_15px_rgba(59,130,246,0.4)]">
        <path d="M200,80 L240,110 L230,160 L260,180 L250,240 L200,280 L140,230 L150,160 L120,130 L160,90 Z" 
              fill="rgba(30, 58, 138, 0.4)" stroke="#3b82f6" strokeWidth="1.5" className="hover:fill-blue-900/60 transition-colors duration-300 cursor-pointer" />
        <path d="M240,110 L280,100 L300,150 L260,180 L230,160 Z" 
              fill="rgba(6, 182, 212, 0.3)" stroke="#06b6d4" strokeWidth="1.5" className="hover:fill-cyan-900/50 transition-colors duration-300 cursor-pointer" />
        <path d="M250,240 L280,260 L240,320 L200,280 Z" 
              fill="rgba(99, 102, 241, 0.3)" stroke="#6366f1" strokeWidth="1.5" className="hover:fill-indigo-900/50 transition-colors duration-300 cursor-pointer" />
      ...
      {/* Grid Floor Effect */}
      <div className="absolute inset-0 bg-[linear-gradient(rgba(59,130,246,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.03)_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_at_center,black_40%,transparent_80%)] pointer-events-none" />
    </div>
  );
};
...
export const DashboardPage = () => {
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-blue-500/30 overflow-hidden flex flex-col relative">
      ...
      {/* Header */}
      <header className="h-20 relative flex items-center justify-between px-8 border-b border-blue-900/30 bg-slate-900/50 backdrop-blur-md z-20">
        <div className="flex items-center gap-4 w-1/4">
          <div className="flex items-center gap-2 bg-slate-800/50 border border-slate-700 rounded px-3 py-1.5 text-xs text-blue-300">
        ...
        <div className="flex-1 flex justify-center relative">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[60px] bg-blue-500/10 blur-[40px] rounded-full pointer-events-none" />
          <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-200 via-white to-cyan-200 tracking-wider uppercase drop-shadow-[0_2px_10px_rgba(59,130,246,0.5)] text-center relative z-10">
            南京 OPC 创业创新专区 · 产业运营看板
          </h1>
          {/* Header Decoration Lines */}
          <div className="absolute bottom-[-20px] left-1/2 -translate-x-1/2 w-[400px] h-[2px] bg-gradient-to-r from-transparent via-blue-500 to-transparent opacity-70" />
          <div className="absolute bottom-[-24px] left-1/2 -translate-x-1/2 w-[200px] h-[2px] bg-gradient-to-r from-transparent via-cyan-400 to-transparent opacity-50" />
        </div>
      ...
      <main className="flex-1 p-6 grid grid-cols-12 gap-6 relative z-10 overflow-hidden">
        ...
        <div className="col-span-3 flex flex-col gap-5 h-full">
          ...
          <Card title="创业态势" icon={Activity} className="h-[32%]">
          ...
          {/* 2. Subject Portrait */}
          <Card title="主体画像" icon={Users} className="h-[32%]">
            <div className="mb-4">
              ...
              <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden flex">
                <div className="h-full bg-blue-500 w-[52%] shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
                 ...
                 <div key={item.name} className="space-y-1">
                   ...
                   <div className="h-1.5 w-full bg-slate-800/50 rounded-full overflow-hidden">
                     <motion.div 
                        ...
                        className={cn(
                          "h-full rounded-full shadow-[0_0_5px_currentColor]",
                          idx === 0 ? "bg-indigo-500 text-indigo-500" :
                          idx === 1 ? "bg-blue-500 text-blue-500" :
                          idx === 2 ? "bg-cyan-500 text-cyan-500" :
          ...
          {/* 3. Model Rankings */}
          <Card title="模型调用量月度 TOP5" icon={Server} className="flex-1">
              ...
              {MODEL_RANK_DATA.map((item, index) => (
                <div key={index} className="flex items-center gap-3 p-2 hover:bg-white/5 rounded-lg transition-colors group cursor-default border border-transparent hover:border-blue-500/20">
                  <div className={cn(
                    ...
                    index < 3 ? "bg-gradient-to-br from-blue-600 to-blue-800 text-white shadow-lg shadow-blue-900/50" : "bg-slate-800 text-slate-500"
                  )}>
                    {item.rank}
                  </div>
        ...
        {/* Center Column */}
        <div className="col-span-6 flex flex-col gap-6 h-full relative">
           {/* Center Header */}
           <div className="absolute top-0 left-0 right-0 h-12 flex items-center justify-center z-10">
              <div className="bg-slate-900/80 backdrop-blur border border-blue-500/30 px-8 py-1.5 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.2)] flex items-center gap-2">
                <Target className="w-4 h-4 text-blue-400 animate-pulse" />
                <span className="text-blue-100 font-semibold tracking-wide">南京 OPC 创业主体分布情况</span>
              </div>
           </div>

           {/* Map Area */}
           <div className="flex-1 relative rounded-2xl border border-blue-900/30 bg-slate-900/20 overflow-hidden group">
              {/* Corner Accents */}
              <div className="absolute top-4 left-4 w-16 h-16 border-t-2 border-l-2 border-blue-500/30 rounded-tl-xl group-hover:border-blue-500/60 transition-colors" />
              <div className="absolute top-4 right-4 w-16 h-16 border-t-2 border-r-2 border-blue-500/30 rounded-tr-xl group-hover:border-blue-500/60 transition-colors" />
              <div className="absolute bottom-4 left-4 w-16 h-16 border-b-2 border-l-2 border-blue-500/30 rounded-bl-xl group-hover:border-blue-500/60 transition-colors" />
              <div className="absolute bottom-4 right-4 w-16 h-16 border-b-2 border-r-2 border-blue-500/30 rounded-br-xl group-hover:border-blue-500/60 transition-colors" />
              
              <MapVisualization />
           </div>

           {/* Bottom Stats */}
           <div className="h-[140px] grid grid-cols-4 gap-4">
              {[
                ...
                <div key={i} className="bg-slate-900/60 border border-slate-800/80 backdrop-blur rounded-xl p-4 flex flex-col justify-between hover:border-blue-500/30 transition-all hover:-translate-y-1 relative overflow-hidden group">
                  ...
                  <div>
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className={`text-2xl font-bold font-mono tracking-tight text-white drop-shadow-md`}>{stat.value}</span>
        ...
        <div className="col-span-3 flex flex-col gap-5 h-full">
           {/* 1. Policy Efficiency */}
           <Card title="政策效能趋势" icon={Box} className="h-[32%]">
           ...
           {/* 2. Policy Process */}
           <Card title="政策申报流程监控" icon={CheckCircle2} className="h-[25%]">
                ...
                {['申请', '审批', '公示', '发放'].map((step, i) => (
                  <div key={step} className="flex flex-col items-center gap-2">
                    <div className={cn(
                      ...
                      i === 3 ? "border-slate-600 text-slate-500" : "border-blue-500 text-blue-400 shadow-[0_0_10px_rgba(59,130,246,0.4)]"
                    )}>
                      {i + 1}
                    </div>
           ...
           <Card title="政策补贴日志" icon={FileText} className="flex-1">
