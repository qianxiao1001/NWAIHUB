      ...
      <div className="flex flex-col lg:flex-row gap-5 items-center justify-between">
        ...
        <div className="flex-1 max-w-2xl">
          ...
          <div className="flex flex-wrap gap-3">
            <Button size="sm" className="bg-indigo-700 hover:bg-indigo-800 text-white shadow-sm px-5 h-8 text-xs">
              立即开通
            </Button>
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4 relative">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center justify-between text-xs p-2 hover:bg-indigo-50/50 rounded-md transition-all duration-200 border border-transparent hover:border-indigo-100 cursor-default">
                  <div className="flex items-center gap-2">
    ...
    <Section className="bg-white py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-5 gap-4">
          <div>
            <h2 className="text-xl font-bold text-slate-900">算力资源市场</h2>
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-0.5 rounded-md border border-slate-200">
            {['全部资源', '训练型', '推理型', '渲染型'].map((tab, i) => (
              <button
                ...
                className={cn(
                  "px-3 py-1 text-xs font-medium rounded-[4px] transition-all duration-200",
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 shadow-sm">
          <table className="w-full text-left border-collapse bg-white">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-200 text-xs font-medium text-slate-500">
                <th className="px-3 py-2">资源规格</th>
                <th className="px-3 py-2">显存</th>
                <th className="px-3 py-2">CPU/内存</th>
                <th className="px-3 py-2">系统盘</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {resources.map((row, idx) => (
                <tr key={idx} className="hover:bg-blue-50/30 transition-colors duration-150 group cursor-default">
                  ...
                  <td className="px-3 py-2.5 text-sm text-slate-600 font-medium">{row.memory}</td>
                  <td className="px-3 py-2.5 text-sm text-slate-500">
                    <div className="flex flex-col">
                      <span className="text-slate-700">{row.cpu}</span>
                      <span className="text-[10px] text-slate-400">{row.ram}</span>
                    </div>
                  </td>
                  <td className="px-3 py-2.5 text-sm text-slate-500">{row.disk}</td>
                  <td className="px-3 py-2.5">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="ui-reveal-group grid md:grid-cols-4 gap-5">
        {[
          ...
          <div key={idx} className="ui-reveal-item bg-white p-4 rounded-xl border border-slate-200 hover:shadow-[0_12px_26px_-22px_rgba(79,70,229,0.45)] hover:border-indigo-200 transition-all duration-200 hover:-translate-y-0.5">
