...
const HeroVisual = () => (
  <div className="rounded-xl bg-gradient-to-br from-slate-800 via-slate-700 to-blue-800 h-[180px] md:h-[200px] relative overflow-hidden">
    ...
    <div className="absolute bottom-4 left-4 text-white">
      <p className="text-lg md:text-xl leading-tight font-bold">{ACTIVITY_ITEMS[0].title}</p>
      <p className="text-[10px] opacity-80 mt-1.5 tracking-[0.12em] uppercase">重点活动</p>
    </div>
  </div>
);
...
const ActivityCard = ({ item }: { item: (typeof ACTIVITY_ITEMS)[number] }) => (
  <article className="bg-white rounded-xl border border-slate-200/30 shadow-sm p-4 transition-all hover:shadow-[0_18px_36px_-26px_rgba(17,28,45,0.55)]">
    ...
    <h3 className="text-lg leading-tight font-bold text-slate-900 mb-2">{item.title}</h3>
    <p className="text-sm text-slate-600 leading-relaxed mb-3">{item.desc}</p>
    <div className="space-y-1.5 text-xs text-slate-500 mb-4">
      <div className="inline-flex items-center gap-1.5"><Clock3 className="w-3.5 h-3.5 text-blue-500" />{item.time}</div>
      <div className="inline-flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-blue-500" />{item.place}</div>
    </div>
    <Button variant="outline" className="w-full h-9 text-xs">查看详情</Button>
  </article>
);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <section className="mb-12 grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
        <div className="lg:col-span-7">
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight leading-tight text-slate-900 mb-4">活动与协作中心</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <aside className="lg:col-span-3 space-y-4">
          <div className="space-y-2">
            {COMMUNITY_SERVICES.map((service) => (
              <button key={service.title} className="w-full flex items-center justify-between rounded-lg bg-[#e9efff] px-3 py-2.5 text-sm font-medium text-slate-800 hover:bg-[#dfe8ff] transition-colors">
          ...
          <div className="rounded-lg border border-slate-200/30 bg-white p-4 shadow-sm">
            <div className="inline-flex items-center gap-1.5 text-xs text-blue-700 font-medium mb-3">
        ...
        <section className="lg:col-span-9 space-y-6">
          <div className="flex items-end justify-between border-b border-slate-200/50 pb-3">
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900">近期活动安排</h2>
